...
public class DatabaseHandler extends SQLiteOpenHelper {
	...
	private static final String KEY_DV_NO = "device_number";
	...
	// Adding new devices
	void addContact(Device device) {
		SQLiteDatabase db = this.getWritableDatabase();

		ContentValues values = new ContentValues();
		values.put(KEY_NAME, device.getName()); // Device Name
		values.put(KEY_DV_NO, device.getDeviceNumber()); // Device ip
	...
	Device getDevice(int id) {
		...
		Cursor cursor = db.query(TABLE_DEVICES, new String[] { KEY_ID,
				KEY_NAME, KEY_DV_NO }, KEY_ID + "=?",
				new String[] { String.valueOf(id) }, null, null, null, null);
		if (cursor != null)
			cursor.moveToFirst();

		Device contact = new Device(Integer.parseInt(cursor.getString(0)),
	...
	// Getting All devices
	public List<Device> getAllContacts() {
		...
		if (cursor.moveToFirst()) {
			do {
				...
				contact.setDeviceNumber(cursor.getString(2));
	...
	// Updating a device detail
	public int updateContact(Device device) {
		SQLiteDatabase db = this.getWritableDatabase();

		ContentValues values = new ContentValues();
		values.put(KEY_NAME, device.getName());
		values.put(KEY_DV_NO, device.getDeviceNumber());
	...
	public void deleteContact(Device device) {
		SQLiteDatabase db = this.getWritableDatabase();
